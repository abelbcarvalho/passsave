...
class InfoUpdate:
    ...
    def __init__(self, info=Info()):
        ...
        self.window.title('PassSave - Atualizar informação extra')
        ...
        self.title_label['text'] = 'Atualize Informação Extra'
        self.title_label.pack()

        self.id_label = Label(self.title_frame)
        self.id_label['font'] = self._font_pequena
        self.id_label['text'] = 'ID Info:'
        self.id_label.pack(side=LEFT)
        
        self.id_info_label = Label(self.title_frame)
        self.id_info_label['font'] = self._font_pequena
        self.id_info_label['text'] = f'{self._info.id}'
        self.id_info_label['bg'] = '#ffff00'
        self.id_info_label.pack(side=LEFT)

        self.fk_label = Label(self.title_frame)
        self.fk_label['font'] = self._font_pequena
        self.fk_label['text'] = 'ID Login:'
        self.fk_label.pack(side=LEFT)

        self.fk_log_label = Label(self.title_frame)
        self.fk_log_label['font'] = self._font_pequena
        self.fk_log_label['text'] = f'{self._info.fk}'
        self.fk_log_label['bg'] = '#ffff00'
        self.fk_log_label.pack(side=LEFT)
        ...
        self.butt_create['text'] = 'Atualizar'
        self.butt_create['bg'] = '#046910'
        self.butt_create['fg'] = '#ffffff'
        self.butt_create['command'] = self._update_operation
    ...
    def _update_operation(self) -> None:
        ...
        if Fac.facade().update_info(info=self._info):
            messagebox.showinfo(title='Info Atualizado',
                                message=Msg.message().mesg)
            self.window.destroy()
        else:
            messagebox.showerror(title='Info Não Atualizado',
                                 message=Msg.message().mesg)
            self.window.destroy()
